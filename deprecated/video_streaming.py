...
class webCamera:
    ...
    def __init__(self, resolution, host):     
    ...
    def _processConnection(self, connection,addr):     
        if(self.recv_config(connection)==0):        
            return    
        ...
        while True:
            ...
            try:                    
                connection.send(struct.pack("img_stream",len(self.imgdata), self.resolution[0],self.resolution[1])+self.imgdata) #发送图片信息(图片长度,分辨率,图片内容)                
            except:            
                f = open("video_info.txt", 'a+')            
    ...
    def run(self):      
        while True: 
            ...
            clientThread = threading.Thread(
            	target = self._processConnection, 
                args = (client, addr, )
                )  #有客户端连接时产生新的线程进行处理
            clientThread.start()
...
def main():      
        cam = webCamera( resolution = (640, 480), host = ("192.168.137.113", 8888))       
        cam.run()  
